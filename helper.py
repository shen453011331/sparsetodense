...
def save_checkpoint(state, is_best, epoch, output_directory, i=0):
    checkpoint_filename = os.path.join(output_directory,
                                       'checkpoint-' + str(epoch) + '-' + str(i) + '.pth.tar')
    ...
    if epoch >= 0:
        prev_checkpoint_filename = os.path.join(
            output_directory, 'checkpoint-' + str(epoch) + '-' + str(i-1000) + '.pth.tar')
