...
parser.add_argument('--savemodel', default='', type=str, metavar='PATH')
...
def iterate(mode, args, loader, model, optimizer, logger, epoch):
    ...
    for i, batch_data in enumerate(loader):
        if i < args.start_iter and epoch == args.start_epoch:
            if i%100 == 0:
                print('jump #{} batch_data'.format(i))
        else:
            start = time.time()
            batch_data = {
                key: val.to(device)
            ...
            if mode == 'train':
                if i % 1000 == 0:
                    helper.save_checkpoint({  # save checkpoint
                        ...
                        'args': args,
                    }, 0, epoch, logger.output_directory, i)
...
def main():
    ...
    if args.evaluate:
        ...
        if os.path.isfile(args.resume):
            ...
            args.data_folder = args_new.data_folder
            args.val = args_new.val
            print("Completed. Resuming from epoch {} iter {}.".format(
